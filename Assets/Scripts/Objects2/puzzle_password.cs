...
public class Puzzle_Passwordkey : MonoBehaviour
{
    ...
    private void HandleMouseClick()
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
        {
            ...
            if (hit.collider != null && hit.collider.gameObject == this.gameObject)
            {
                ...
                if(CheckPassword(info.puzzles_answer[info.puzzle_num]))
                {
                    info.puzzles[info.puzzle_num] = true;
    ...
    bool CheckPassword( List<int> password )
    {
